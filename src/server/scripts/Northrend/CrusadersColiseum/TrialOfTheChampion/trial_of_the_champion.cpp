...
class npc_announcer_toc5 : public CreatureScript
{
    ...
    struct npc_announcer_toc5AI : public ScriptedAI
    {
        ...
        void NextStep(uint32 uiTimerStep, uint32 currentEvent, bool bNextStep = true, uint32 uiPhaseStep = 0)
        {
            if (bNextStep)
                events.ScheduleEvent(currentEvent + 1, Seconds(uiTimerStep));
            else
            {
                if (uiPhaseStep > 0)
                    events.ScheduleEvent(uiPhaseStep, Seconds(uiTimerStep));
            }
        }
        ...
        void SetData(uint32 uiType, uint32 uiData) override
        {
            switch (uiType)
            {
                case DATA_START:
                    ...
                    NextStep(6, 0, false, EVENT_CHAT_9);
                    break;
                ...
                case DATA_GRAND_CHAMPIONS_DONE:
                    NextStep(3, 0, false, EVENT_CHAT_8);
                    break;
                case DATA_ARGENT_CHAMPION_PREPARE:
                    NextStep(1, 0, false, EVENT_GO_TO_ARGENT_BOSS);
                    break;
                case DATA_BLACK_KNIGHT_PREPARE:
                    NextStep(2, 0, false, EVENT_CHAT_15);
                    break;
                case DATA_BLACK_KNIGHT_PRECAST:
                    NextStep(1, 0, false, EVENT_FACING_2);
                    break;
                case DATA_BLACK_KNIGHT_DONE:
                    NextStep(7, 0, false, EVENT_CHAT_20);
                    break;
                default:
                    break;
            }
        ...
        void MovementInform(uint32 uiType, uint32 uiPointId) override
        {
            ...
            switch (uiPointId)
            {
                case POINT_ENCOUNTER_1_WAIT:
                    NextStep(0, 0, false, EVENT_WAIT_1);
                    break;
                ...
                case POINT_START_ARGENT_CHAMP:
                    ...
                    NextStep(0, 0, false, EVENT_SPAWN_ALL);
                    break;
                case POINT_ENCOUNTER_2_WAIT_1:
                    NextStep(0, 0, false, EVENT_WAIT_2);
                    break;
                case POINT_ENCOUNTER_2_WAIT_2:
                    me->SetFacingTo(centerOrientation);
                    NextStep(0, 0, false, EVENT_CHAT_12);
                    break;
                case POINT_ENCOUNTER_3_WAIT:
                    me->SetFacingTo(centerOrientation);
                    NextStep(0, 0, false, EVENT_CHAT_13);
                    break;
                default:
                    break;
            }
        ...
        void DoStartArgentChampionEncounter()
        {
            ...
            if (GameObject* cache = instance->GetGameObject(DATA_CHAMPION_S_CACHE))
                cache->Delete();
            NextStep(1, 0, false, EVENT_MOVE_MIDDLE);
        }
        ...
        void DoStartBlackKnight()
        {
            ...
            NextStep(1, 0, false, EVENT_STEP_FORWARD);
        }
        ...
        void StartEncounter(bool startRp = false)
        {
            ...
            if (instance->GetBossState(DATA_BLACK_KNIGHT) == NOT_STARTED || instance->GetBossState(DATA_BLACK_KNIGHT) == TO_BE_DECIDED)
            {
                if (instance->GetBossState(DATA_ARGENT_CHALLENGE) == NOT_STARTED || instance->GetBossState(DATA_ARGENT_CHALLENGE) == TO_BE_DECIDED)
                {
                    ...
                    {
                        Talk(SAY_INTRO_1);
                        NextStep(7, 0, false, EVENT_INTRODUCE);
                    }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_CHEER_RND:
                        if (events.GetTimeUntilEvent(0) == 0s && !me->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP) && !me->isMoving() && !me->HasAura(66804))
                        {
                    ...
                    case EVENT_CLOSE_DOOR_3:
                        ...
                        instance->HandleGameObject(instance->GetGuidData(DATA_MAIN_GATE), false);
                        me->GetMotionMaster()->MovePoint(POINT_ENCOUNTER_1_WAIT, announcerEncounterPos);
                        NextStep(1, 0, false); // MovementInform continues from this
                        break;
                    case EVENT_WAIT_1:
